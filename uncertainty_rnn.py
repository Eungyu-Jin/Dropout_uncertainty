...
def dataset(label = '역률평균'):
    ...
    idx = df['TIMESTAMP'].unique()
    ds = pd.DataFrame(index = idx, columns=features)
    for i in features:
        ds[i] = df[df['ITEM_NAME']==i][['ITEM_VALUE', 'LABEL_NAME']].values
...
def preprocess(df, label = '역률평균', timesteps = 12, predict_size = 1):
    # 6시간(timesteps) 전의 데이터를 참고 해 1시간 뒤(predict_size)를 예측하는 dataset을 생성 - train 데이터
...
class Current():
    def __init__(self, df):
        ...
        self.dropout_rate = 0.25 # 랜덤으로 가지치기 할 비율
    ...
    def Transfomer_LSTM(self, key_dim = 256, num_heads=2, ff_dim=128, num_blocks=1):
        # 인코더는 LSTM, 디코더는 트랜스포머인 모델
        inputs = Input(shape=self.input_shape)
    ...
    def Transfomer_Conv(self, key_dim = 256, num_heads=2, ff_dim=128, num_blocks=1):
        # 인코더는 CNN, 디코더는 트랜스포머인 모델
        inputs = Input(shape=self.input_shape)
        x = Conv1D(filters=self.units, kernel_size=self.train_X.shape[1], strides = 1, activation = 'relu', padding='causal',dilation_rate=4)(inputs, training = True)
        x = Dropout(0.25)(x, training = True)
        x = Conv1D(filters=self.units, kernel_size=self.train_X.shape[1], strides = 1, activation = 'relu', padding='causal',dilation_rate=16)(x, training = True)
        x = Dropout(0.25)(x, training = True)
    ...
    def predict_ci(self, model, n_sample, ci=2.58, show= True):
        ...
        if show == True:
            ...
            plt.xlabel('Time')
            plt.ylabel('Test')
            plt.legend(['Test','Prediction','Uncertainty'],loc='upper right')
        ...
        tf_predict['sigma_upper'] = sigma_upper
        tf_predict['sigma_lower'] = sigma_lower

        # 예측 데이터의 임계값 계산
        test_loss = np.square(y_mean_transfrom - test_y_transfrom)
        test_loss_ewm = pd.DataFrame(test_loss).ewm(alpha= 0.5).mean()
        ewm_upper = np.mean(test_loss_ewm) + 3 * np.std(test_loss_ewm)
        ewm_lower = np.mean(test_loss_ewm) - 3 * np.std(test_loss_ewm)
        tf_predict['ewm_upper'] = ewm_upper
        tf_predict['ewm_lower'] = ewm_lower
